...
export function Sidebar({ onClose }: SidebarProps) {
  ...
  const usagePct =
    episodesLimit > 0 ? Math.round((episodesUsed / episodesLimit) * 100) : 0;
  const isNearLimit = usagePct >= 80;

  return (
    <div className="flex h-full flex-col bg-card border-r border-border">
      {/* Logo */}
      <div className="flex h-14 items-center gap-1 px-4 border-b border-border">
        <Image src="/logo.png" alt="Expandcast" width={32} height={32} className="object-contain" />
        ...
        <div className="border-t border-border p-3">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="ghost"
                className="w-full h-auto px-2 py-2 justify-start"
              >
                <div className="flex items-center gap-3 w-full">
                  ...
                  <div className="flex flex-col items-start flex-1 min-w-0">
                    ...
                    <span
                      className={cn(
                        "text-xs capitalize px-1.5 py-0.5 rounded font-medium",
